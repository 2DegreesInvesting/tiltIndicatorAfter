add_coefficient_of_variation_transition_risk <- function(data) {
  ...
  company <- data |>
    unnest_company() |>
    add_coefficient_of_variation_transition_risk_impl() |>
    polish_coefficient_of_variation_transition_risk()

  tilt_profile(nest_levels(product, company))
}

add_coefficient_of_variation_transition_risk_impl <- function(data) {
  data |>
    add_mean_cov(
    ...
    add_coefficient_of_variation(
      "coefficient_of_variation_transition_risk",
      "standard_deviation_transition_risk",
      "mean_transition_risk"
    )
}
