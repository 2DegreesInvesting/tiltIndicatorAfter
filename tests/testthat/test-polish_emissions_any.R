...
test_that("outputs co2e* by default", {
  ...
  expect_named(polish_emissions_any(data), c(col_min_jitter(), col_max_jitter()))
})

test_that("can output `min`, `max`", {
  data <- tibble(min = 1, max = 2, min_jitter = 0, max_jitter = 4)

  out <- polish_emissions_any(data, output_min_max = TRUE)
...
test_that("can output `output_co2_footprint`", {
  ...
  expect_named(
    polish_emissions_any(data, output_co2_footprint = TRUE),
